...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (!loading && user && userData && !error && !accountExists) {
  ...
  }, [user, userData, loading, router, error, accountExists]);
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Check if user's email is in any school's authorized admin list
      ...
      // Route to appropriate step based on role and authorization
      if (authorizedSchool) {
        // User is authorized for admin access - create account directly
        if (role === "admin") {
          await createUserAccount(tempUserData, "admin", authorizedSchool.id);
        } else {
          // They selected teacher/student but are authorized for admin
          setError(`You are authorized for admin access at ${authorizedSchool.name}. Please select "School Administrator" as your role.`);
          setTempUser(null);
          return;
        }
  ...
  const handleCreateSchool = async () => {
    if (!tempUser || !schoolDetails.name.trim()) return;
    ...
    try {
      ...
      for (const schoolDoc of schoolsSnapshot.docs) {
        const schoolData = schoolDoc.data();
        const authorizedEmails = schoolData.authorizedAdminEmails || [];
        if (authorizedEmails.includes(tempUser.email.toLowerCase())) {
          authorizedSchool = { id: schoolDoc.id, ...schoolData };
          break;
        }
      }
      ...
      if (authorizedSchool) {
        ...
        await createUserAccount(tempUser, "admin", authorizedSchool.id);
      } else {
        // Create new school
        ...
        const schoolData = {
          ...
          teacherJoinCode: Math.random().toString(36).substring(2, 8).toUpperCase(),
          createdBy: tempUser.uid,
          createdAt: Date.now(),
          authorizedAdminEmails: [tempUser.email.toLowerCase()], // Add creator to authorized list
        };
        ...
        await createUserAccount(tempUser, "admin", schoolId);
      }
    } catch (error) {
    ...
    try {
      ...
      if (joinTypeSetting === "manual") {
        ...
        await addDoc(collection(firestore, "schoolJoinRequests"), {
          studentId: tempUser.uid,
          schoolId: schoolId,
          studentName: tempUser.displayName || tempUser.email,
          studentEmail: tempUser.email,
          status: "pending",
          createdAt: new Date(),
        ...
        await createUserAccount(tempUser, role, null);
        ...
        await createUserAccount(tempUser, role, schoolId);
      }
    } catch (err) {
  ...
  if (user && userData && !accountExists && !error) {
    return null;
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-muted p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Join StudyHub</h1>
          ...
          {step === "role" ? (
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full btn-primary flex justify-center items-center gap-3 py-4 text-lg"
              >
                <img
